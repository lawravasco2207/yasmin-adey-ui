...
const ContentManagement: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const refreshedResponse = await getContent();
  ...
  const handleAddLink = async () => {
    ...
    try {
      const response = await addLink(newLink.name, newLink.url);
  ...
  return (
    <motion.div
      className="futuristic-bg content-management"
    ...
    >
      ...
      >
        ...
        <input
          ...
          placeholder="Caption (e.g., **bold**, *italic*, [Brand](https://...))"
        />
          ...
          {brandLinks.map((link, index) => (
            <div key={index} className="brand-link-input">
              <input
                type="text"
                value={link.name}
                onChange={(e) => handleBrandLinkChange(index, 'name', e.target.value)}
                placeholder="Brand Name"
              />
              <input
                type="text"
                value={link.url}
                onChange={(e) => handleBrandLinkChange(index, 'url', e.target.value)}
                placeholder="URL (e.g., https://brand.com)"
              />
              <motion.button
      ...
      >
        ...
        <input
          ...
          placeholder="Link Name (e.g., Shop my outfit at Amazon)"
        />
        <input
          type="text"
          value={newLink.url}
        ...
        <ul>
          {contentList.map((item) => (
            ...
            >
              {item.type === 'image' && (
                <LazyLoadImage
                  src={`http://localhost:5000${JSON.parse(item.file_path)[0]}`}
                  alt={item.title}
                  effect="blur"
                  className="thumbnail"
                />
              ...
              {item.type === 'slideshow' && (
                <div className="thumbnail-slideshow">
                  {JSON.parse(item.file_path).map((path: string, index: number) => (
                    <LazyLoadImage
                      key={index}
              ...
              <div className="content-details">
                ...
                {item.file_path && (
                  <a href={`http://localhost:5000${JSON.parse(item.file_path)[0]}`} target="_blank" rel="noopener noreferrer">
                    Peek
                  </a>
                )}
              </div>
